...
import wda
from common.ios_proxy import ProxyForwarder


class BasePage:
    def __init__(self, server, is_mobile_device=True, is_ios=True, device_id=None, is_record=False):
        self.server = server

        # 是否在手机, PC-浏览器需要改为False
        ...
        if self.is_mobile_device:
            if self.is_ios:
                self._init_device_ios()
            else:
                self._init_device_android()
        else:
            self._init_webdriver_chrome()
    ...
    def _init_device_android(self):
        """安卓设备执行：初始化"""
        self.client = AdbClient(host="127.0.0.1", port=5037)
        print("设备：",self.client.devices())
        if self.device_id:
        ...
        self.screen_w, self.screen_h = map(int, output.split(':')[-1].strip().lower().split('x'))
        # 获取屏幕方向
        # self.rotation = int(self.device.shell("dumpsys input | grep SurfaceOrientation").split()[-1]) % 4

    def _init_device_ios(self):
        """苹果设备执行：初始化"""
        # 将设备的 8100 端口（wda）转发到本地
        self.forwarder = ProxyForwarder()
        local_port, _ = self.forwarder.setup_proxy(8100)

        if not self.device_id:
            self.device_id = wda.usbmux.pyusbmux.list_devices()[0].serial
        self.device = wda.Client(f"http://localhost:{local_port}")
        print("设备状态:", self.device.status())

        window_size = self.device.window_size()
        self.screen_w = window_size[0]
        self.screen_h = window_size[1]

    def _init_webdriver_chrome(self):
        """浏览器执行：初始化"""
        # 配置 Chrome 调试参数
    ...
    async def press_position_base(self, position, duration: float = 2.0):
        ...
        if self.is_mobile_device:
            position[0] = int(position[0] * self.screen_w)
            position[1] = int(position[1] * self.screen_h)
            if duration == 0.1:
                if self.is_ios:
                    self.device.click(position[0], position[1])
                else:
                    self.device.shell(f"input tap {position[0]} {position[1]}")
            else:
                duration = duration * 1000
                if self.is_ios:
                    self.device.tap_hold(position[0], position[1], duration)
                else:
                    self.device.shell(f"input swipe {position[0]} {position[1]} {position[0]} {position[1]} {duration}")
        else:
    ...
    async def swipe_base(self, point_start=None, point_end=None, t: float = 0.5):
        ...
        if self.is_mobile_device:
            point_start[0] = int(point_start[0] * self.screen_w)
            point_start[1] = int(point_start[1] * self.screen_h)
            point_end[0] = int(point_end[0] * self.screen_w)
            point_end[1] = int(point_end[1] * self.screen_h)
            t = t * 1000
            if self.is_ios:
                self.device.swipe(point_start[0], point_start[1], point_end[0], point_end[1], t)
            else:
                self.device.shell(f"input swipe {point_start[0]} {point_start[1]} {point_end[0]} {point_end[1]} {t}")
        else:
            ...
            self.driver.execute_script(drag_script, web_position_start[0], web_position_start[1],
                                       web_position_end[0], web_position_end[1], t*1000)
    ...
    async def click_object_of_plural_objects(self, element_data: dict = None, element_data_list: list = None,
        ...
        print("click_object_of_plural_objects:", element_data)
        if uuid:
    ...
    async def click_element_safe(self, uuid: str = None, element_data: dict = None, offspring_path="",
                                 anchor_point: list = None, locator_camera: str = "",log: bool = True):
        """函数功能简述
            元素存在且在屏幕范围内时再点击
            不清除弹窗
        ...
        try:
            ...
            pass
        if log:
            print("click_element_safe 成功: ", element_data)
    ...
    async def click_a_until_b_appear(self, element_data_a: dict, element_data_b: dict,
        ...
        while not await self.exist(element_data=element_data_b):
            await self.click_element_safe(element_data=element_data_a,log=False)
            await self.clear_popup(ignore_set=ignore_set)
            await self.sleep(interval)

    async def click_a_until_b_appear_list(self, perform_list: list, interval: float = 0.5,timeout:int=500000):
        """函数功能简述
        ...
        while cur < len(perform_list) - 1:
            print("{} / {}".format(cur + 1, len(perform_list) - 1))
            if self.is_ios and interval >= 0.2:
                interval = interval - 0.3 if interval >= 0.5 else 0.1

            try:
                await asyncio.wait_for(
                    self.click_a_until_b_appear(element_data_a=perform_list[cur],
                                                element_data_b=perform_list[cur + 1],
                                                interval=interval),
                    timeout=timeout
                )
            except asyncio.TimeoutError:
                print(f"第 {cur + 1} 步超时（>{timeout}s）！点击: {perform_list[cur]} → 等待: {perform_list[cur + 1]}出现失败")
                print("脚本运行失败")
                raise
    ...
    async def click_until_disappear(self, element_data: dict = None, interval: float = 1, ignore_set=None,timeout: float = 300):
        """函数功能简述
        ...
        try:
            await asyncio.wait_for(
                self.click_a_until_b_disappear(
                    element_data_a=element_data,
                    element_data_b=element_data,
                    interval=interval,
                    ignore_set=ignore_set
                ),
                timeout=timeout
            )
            print("click_until_disappear 成功:", element_data)
        except asyncio.TimeoutError:
            print(f"[超时] 等待元素消失超过 {timeout} 秒: {element_data}")
            print("脚本运行失败")
            raise

    async def wait_for_appear(self, element_data: dict = None, element_data_list=None, is_click: bool = False,
                              interval: float = 0.5, timeout=300, ignore_set=None):
        """函数功能简述
            等待元素出现
