...
class ProxyForwarder(object):
    ...
    def usb_device(self):
        ...
        """
        if not self._device:
            self._udid = wda.usbmux.pyusbmux.list_devices()[0].serial
            usb_dev = wda.Client(f"http+usbmux://{self._udid}:8100")
            try:
                if usb_dev.device_info()['uuid'] == self.uuid:
                    self._device = wda.usbmux.select_device(udid=self._udid)
    ...
    def do_proxy(self, port, device_port):
        ...
        if proxy_process:
            ...
            # self.do_proxy_usbmux(5001, 5001)
            return
