...
def rpc_server(port=None):
    """
    RPC服务器装饰器
    支持动态端口配置
    Args:
        port: 端口号，如果为None则使用配置的端口
    """
    def decorator(handler):
        ...
        def run_server():
            async def server_main():
                ...
                # 使用传入的端口或配置的端口
                actual_port = port if port is not None else get_port()
                
                # 显示端口配置信息
                print(get_port_info())
                
                async with websockets.serve(lambda ws: wrapped_handler(ws, stop_event), "0.0.0.0", actual_port):
                    print(f"RPC Server running at ws://0.0.0.0:{actual_port}")
