...
async def chapter_five(bp: BasePage):
    ...
    perform_list_17 = [
        ...
        ElementsData.UISlgBuildingUpgrade.use_diamond,
        ElementsData.UIBuilding.task_effect, 
        ElementsData.UiWechatPushGuide.close,#10     #微信小程序订阅弹窗关闭
        ElementsData.UiWechatPushGuideConfirm.cancel,#15     #微信小程序订阅弹窗取消
        ElementsData.UIChapterChangeTime.tap_close,#20
        ElementsData.UIBuildingUnlock.lab_continue,
        ElementsData.UILevelHUD.xin_shou,
        ElementsData.UiMission.btn_claim,
        ElementsData.UIReward.lab_continue,
...
def config_limit():
    parser = argparse.ArgumentParser(description='自动测试配置脚本')
    parser.add_argument('--max_chapter',type=int,choices=range(1,8),help='测试到第几章节(1-7),不输入默认测试到第七章')
    parser.add_argument('--device-id',type=str,help='设备ID')
    parser.add_argument('--mobile',action='store_true',help='是否在移动端运行')
    parser.add_argument('--device_type',type=str,choices=['android','ios'],help='设备类型')
    parser.add_argument('--ws-port',type=int,help='WebSocket端口')
    parser.add_argument('--skip-chapters',type=str,help='跳过的章节，用逗号分隔')
    
    args, unknown = parser.parse_known_args()
 
    # 设置默认值
    max_chapter = args.max_chapter if args.max_chapter is not None else 7
    ...
    print(f"使用命令行参数: 测试到第{max_chapter}章节")
    if device_id:
        print(f"使用设备id: {device_id}")
    if is_mobile:
        print("在移动端运行")
    if device_type:
        print(f"使用设备类型: {device_type}")
    if ws_port:
        print(f"使用WebSocket端口: {ws_port}")
    if skip_chapters:
        print(f"跳过章节: {skip_chapters}")
    
    return max_chapter, device_id, is_mobile, device_type, ws_port


    # while True:
    #     user_input = input("请输入测试到第几章节(1-7),不输入默认测试到第七章:").strip()
    ...
    #         else:
    #             print("输入的章节号不在1-7范围内，请重新输入")
    #     except ValueError:
    #         print("输入的章节号不是数字，请重新输入")

async def run_chapter(bp:BasePage,max_chapter):
    chapter_functions = {
        1 : chapter_one,
        2 : chapter_two,
        3 : chapter_three,
    ...
    for chapter_num in range(1,max_chapter + 1):
        if chapter_num in chapter_functions:
            print(f"=================第{chapter_num}章节开始================")
            start_time = datetime.now()
            await chapter_functions[chapter_num](bp)
            print(f"=================第{chapter_num}章节结束================")
            end_time = datetime.now()
            duration = end_time - start_time
            print(f"第{chapter_num}章节开始时间: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
            print(f"第{chapter_num}章节结束时间: {end_time.strftime('%Y-%m-%d %H:%M:%S')}")
            print(f"第{chapter_num}章节总耗时: {str(duration)}")
        else:
            print(f"=================第{chapter_num}章节不存在================")
...
async def main(server):
    ...
    max_chapter, device_id, is_mobile_device, device_type, ws_port = config_limit()
    ...
    # 日志系统已由多设备执行器统一管理，无需在此处创建
    # 从环境变量获取设备日志文件路径
    import os
    log_file = os.environ.get('DEVICE_LOG_FILE')
    ...
    try:
        ...
        await run_chapter(bp,max_chapter)
