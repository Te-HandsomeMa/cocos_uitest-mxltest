...
def config_limit():
    ...
    parser.add_argument('--device_id',type=str,help='设备ID')
    ...
    if args.max_chapter is not None:
        print(f"使用命令行参数: 测试到第{args.max_chapter}章节")
        if args.device_id:
            print(f"使用设备id: {args.device_id}")
        if args.mobile:
            print("在移动端运行")
        if args.device_type:
            print(f"使用设备类型: {args.device_type}")
...
async def main(server):
    # ...existing code...
    print(get_port_info())
    max_chapter, device_id, is_mobile_device, device_type = config_limit()
